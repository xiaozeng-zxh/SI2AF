...
class Agent(object):
    ...
    def __init__(self, a_id, env, data, ctrled_accounts, all_targets, non_targets, idx_test, idx_train, num_wrong=0):
        self.a_id = a_id
        self.data = data
        ...
        self.all_non_targets = defaultdict(list)
        for t in idx_train + idx_test:
            self.ctrled_accounts[t] = deepcopy(ctrled_accounts)
            self.all_non_targets[t] = deepcopy(all_targets) + deepcopy(non_targets)
        ...
        self.snet = QNetNode(self.data, deepcopy(self.all_non_targets))
        self.old_snet = QNetNode(self.data, deepcopy(self.all_non_targets))
        ...
        if args.device != "cpu":
            ...
            self.snet = self.snet.to(args.device)
            self.snet.data.to(args.device)
            self.old_snet = self.old_snet.to(args.device)
            self.old_snet.data.to(args.device)
    ...
    def take_snapshot(self):
        ...
        self.old_snet.load_state_dict(self.snet.state_dict())
    ...
    def make_actions(self, time_t, target_list, greedy=False):
        ...
        # if not greedy:
            ...
            with torch.no_grad():
                actions_1, values_2 = self.net(time_t, cur_state, None, greedy_acts=True)
                actions_2, values_2 = self.snet(time_t, cur_state, None, greedy_acts=True)
            actions_1 = list(actions_1.cpu().numpy())
            actions_2 = list(actions_2.cpu().numpy())
            actions = list()
            for i in range(len(actions_1)):
                actions.append([actions_1[i], actions_2[i]])
            # print(actions)
            # print(sum([self.account_type[a] for a in actions]) / len(target_list))
            # exit()
    ...
    def run_simulation(self):
        ...
        self.snet.list_action_spcae = deepcopy(self.all_non_targets)
    ...
    def train(self):
        ...
        for self.step in pbar:
            ...
            if not list_term[0]:
                ...
                _, q_rhs_1 = node_greedy_actions(target_nodes, q_t_plus_1, self.old_net)
                _, q_t_plus_2 = self.old_snet(curtime + 1, list_s_primes, None)
                _, q_rhs_2 = node_greedy_actions(target_nodes, q_t_plus_2, self.old_snet)
                list_target += (q_rhs_1 + q_rhs_2)
            ...
            self.net.list_action_space = deepcopy(self.ctrled_accounts)
            self.snet.list_action_spcae = deepcopy(self.all_non_targets)
            list_at_1, list_at_2 = list(), list()
            for la in list_at:
                list_at_1.append(la[0])
                list_at_2.append(la[1])
            _, q_sa_1 = self.net(cur_time, list_st, list_at_1)
            q_sa_1 = torch.cat(q_sa_1, dim=0)
            _, q_sa_2 = self.snet(cur_time, list_st, list_at_2)
            q_sa_2 = torch.cat(q_sa_2, dim=0)
            # print(q_sa)
            loss = F.mse_loss(q_sa_1 + q_sa_2, list_target)
            optimizer.zero_grad()
            loss.backward()
