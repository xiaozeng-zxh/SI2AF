...
acc_select = []
for acc in acc_candidate.values():
    if len(acc) <= 50:
        acc_select += acc
    else:
        acc_select += list(rd.sample(acc, min(len(acc), 500)))

controlled_user = user_selection(acc_select, 3)
...
if __name__ == "__main__":
    surrogate_model.load_state_dict(
        torch.load(f"./Detector/trained_model/{args.dataset[:3]}_{args.base_model}_hop_{args.nhop}.pth", map_location=torch.device('cpu')))
    ...
    partition_dict, prob_dict, agg_ps = news_partition(target_news_list, controlled_user, prob_log.tolist(), 5)  # height=5
    ...
    for idx, target_news in enumerate(target_news_list):
        ...
        all_targets, non_targets = partition_dict[target_news][1], partition_dict[target_news][2]
        controlled_user = partition_dict[target_news][0]
        ctrled_user_list = sum([user for user in controlled_user], [])
        ...
        center_nodes = ctrled_user_list + all_targets + non_targets
        center_nodes = torch.tensor(center_nodes, dtype=torch.long)

        new_target = len(all_new_accounts)
        all_targets = list(range(len(all_new_accounts), len(all_new_accounts) + len(all_targets)))
        non_targets = list(range(len(all_new_accounts) + len(all_targets), len(all_new_accounts) + len(all_targets) + len(non_targets)))
        ...
        env = NodeAttakEnv(new_data, agent_ctrl_accounts, target_news, 
                           all_targets, non_targets, surrogate_model)

        agents = dict()
        for a_id, accounts in agent_ctrl_accounts.items():
            agents[a_id] = Agent(a_id, env, q_net_data, accounts, all_targets, non_targets, meta_list, attack_list,
                                 num_wrong=len(correct) - sum(correct))

        center_agent = CenterAgent(env, q_net_data, agent_ctrl_accounts, all_targets, non_targets, meta_list, attack_list, agents, agg_ps=agg_ps[target_news],
                                   num_wrong=len(correct) - sum(correct))
        center_agent.train()
...
succ_rate = len(news_attack_succ) / len(target_news_list)
