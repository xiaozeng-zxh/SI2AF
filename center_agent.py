...
class CenterAgent(object):
    # def __init__(self, env, data, ctrled_accounts, idx_test, idx_train, agents, agg_ps, num_wrong=0):
    def __init__(self, env, data, ctrled_accounts, all_targets, non_targets, idx_test, idx_train, agents, agg_ps, num_wrong=0):
        self.data = data
        ...
        self.agent_all_non_targets = defaultdict(dict)
        ...
        for a_id in self.agents:
            ...
            agent_all_non_targets = defaultdict(list)
            for t in idx_train + idx_test:
                agent_ctrled_accounts[t] = deepcopy(ctrled_accounts[a_id])
                agent_all_non_targets[t] = deepcopy(all_targets) + deepcopy(non_targets)
            self.agent_ctrled_accounts[a_id] = agent_ctrled_accounts
            self.agent_all_non_targets[a_id] = agent_all_non_targets
    ...
    def action_agg(self, agent_at, agent_mapping):
        ...
        for act in range(len(agent_at[0])):
            agent = np.random.choice(num_agent, p=[0.05, 0.05, 0.9])  # [0.05, 0.05, 0.9]
    ...
    def run_simulation(self):
        ...
        for a_id, all_non_targets in self.agent_all_non_targets.items():
            self.agents[a_id].snet.list_action_space = deepcopy(all_non_targets)
    ...
    def eval(self):
        ...
        for a_id, agent in self.agents.items():
            ...
            agent.snet.list_action_space = deepcopy(
                self.agent_all_non_targets[a_id])
        t = 0
        ...
        if args.phase == 'train':
            if self.best_eval is None or acc < self.best_eval:
                for a_id, agent in self.agents.items():
                    self.best_model[a_id] = agent.net.state_dict()
                self.modified_list = self.env.modified_list
                self.best_binary_rewards = self.env.binary_rewards
                self.best_eval = acc
                self.prob = prob
        return acc
    ...
    def train(self):
        ...
        for self.step in pbar:
            ...
            for a_id, mem_id in mem_agent_mapping.items():
                ...
                selected_list_at_1, selected_list_at_2 = list(), list()
                for la in selected_list_at:
                    selected_list_at_1.append(la[0])
                    selected_list_at_2.append(la[1])
                _, q_sa_1 = agent.net(
                    cur_time, selected_list_st, selected_list_at_1)
                q_sa_1 = torch.cat(q_sa_1, dim=0)
                _, q_sa_2 = agent.snet(
                    cur_time, selected_list_st, selected_list_at_2)
                q_sa_2 = torch.cat(q_sa_2, dim=0)
                # print(q_sa)
                loss = F.mse_loss(q_sa_1 + q_sa_2, agent_list_target[a_id])
        ...
        with open(args.save_dir + '/epoch-best.txt', 'a') as f:
            f.write('acc: %.4f\n' % self.best_eval)
        with open(args.save_dir + '/attack_solution.txt', 'a') as f:
            for i in range(len(self.idx_train)):
                f.write('%d: [' % self.env.target_news)
                for e in range(self.modified_list[i].added_edges.shape[1]):
                    f.write(
                        f'({self.env.modified_list[i].added_edges[0, e]} {self.env.modified_list[i].added_edges[1, e]})')
                f.write('] prob: %f\n' % (self.prob))
