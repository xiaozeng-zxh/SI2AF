...
class NodeAttakEnv(object):
	# def __init__(self, data, ctrled_accounts, target_news, all_targets, classifier):
	...
	def __init__(self, data, ctrled_accounts, target_news, all_targets, non_targets, classifier):
		self.classifier = classifier
		self.all_targets = all_targets
		...
		for a_id in ctrled_accounts:
			...
			for t in all_targets:
				agent_all_non_targets[t] = deepcopy(self.all_targets) + deepcopy(self.non_targets)
	...
	def setup(self, target_nodes):
		...
		self.list_saction_space = deepcopy(self.agent_all_non_targets)
	...
	def step(self, actions, eval_flag=False):
		...
		for i in range(len(self.target_nodes)):
			...
			# direct + feedback attacks
			# if len(self.all_targets) == 1:
			# 	self.modified_list[i].add_edge(actions[i][0], actions[i][1])
			# else:
			# 	if actions[i][1] <= self.all_targets[-1]:
			# 		self.modified_list[i].add_edge(actions[i][0], actions[i][1])
			# 	else:
			# 		self.modified_list[i].add_edge(self.target_nodes[i], actions[i][0])
		...
		if self.isTerminal():
			...
			pred_log, label_log, prob_log, correct, loss = eval(
				self.classifier, temp_new_data.to(args.device), self.all_targets)
	...
	def sample_pos_rewards(self, num_samples):
		...
		for i in range(len(self.list_acc_of_all)):
			succ = np.where(self.list_acc_of_all[i] < 0.9)[0]  # < 0.9
	...
	def uniformRandActions(self, a_id):
		...
		for i in range(len(self.target_nodes)):
			...
			region_1 = self.list_action_space[a_id][cur_node]
			region_2 = self.list_saction_space[a_id][cur_node]
			cur_action_1 = region_1[np.random.randint(len(region_1))]
			cur_action_2 = region_2[np.random.randint(len(region_2))]
			# act_list.append(cur_action)
			act_list.append([cur_action_1, cur_action_2])
		return act_list
